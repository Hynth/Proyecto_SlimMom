...
const Modal = ({ values, onClose }) => {
  ...
  const dailyRate =
    ...
    6.25 * formValues.height -
    5 * formValues.age -
    161 -
    10 * (formValues.weight - formValues.desiredWeight);

  let notAllowedProducts = filterObjectsByGroupBloodNotAllowed(
    products,
      ...
      <div className={styles.modalContent}>
          ...
          <ul className={styles.list}>
            {notAllowedProductsList.map((item, index) => (
              <li key={index} className={styles.items}>
                {index + 1 + '. ' + notAllowedProductsList[index]}
              </li>
            ))}
          </ul>
        </div>
        <div className={styles['button-container']}>
          <button className={styles['start-button']} type="submit">
            <Link to="/register">Comienza a perder peso</Link>
          </button>
